...
namespace RFC
{
    ...
    {
        private readonly System.Timers.Timer m_Time = new System.Timers.Timer();
        ...
        {
            Win32API.GetWindowText(p_Handle, out Win32API.STRINGBUFFER _TextString, 256);
            Win32API.GetClassName(p_Handle, out Win32API.STRINGBUFFER _ClassName, 255);
        ...
        public void Hide(IntPtr p_ListViewIntPtr)
        {
            ...
            for (int z = 0; z != _ItemCount; z++)
            {
                ...
                //把数据传递给结构体 (LVITEM)    
                Win32API.WriteProcessMemory(_ProcessIntPtr, _Memory, _OutBytes, (uint)_OutBytes.Length, out _);

                //发送消息获取结构体数据  
                Win32API.SendMessage(p_ListViewIntPtr, 0x102D, z, _Memory);
                //获取结构体数据  
                Win32API.ReadProcessMemory(_ProcessIntPtr, _Memory, _OutBytes, (uint)_OutBytes.Length, out _);

                //获取结构体 pszText的地址  
                IntPtr _ValueIntPtr = new IntPtr(BitConverter.ToInt32(_OutBytes, 20));

                byte[] _TextBytes = new byte[255];  //获取pszText的数据  
                Win32API.ReadProcessMemory(_ProcessIntPtr, _ValueIntPtr, _TextBytes, 255, out _);
                //获取进程名称   
